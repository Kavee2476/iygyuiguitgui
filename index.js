/*
	...
	あ GK KASI CREDIT YT кαвєє σƒ¢, GK USAH RE UPLOAD
	...
	あ JASA RUN HEROKU BISA PC wa.me/94755183063 BIAR BOT NYA GAK TERLALU DELAY
...
gc_wa_lu = 'https://chat.whatsapp.com/CppkFM8XbA28mIjZDTziJx' //klo gk punya gc wa gk usah di ganti 👍
...
let _votes = JSON.parse(fs.readFileSync(`./lib/vote/${from}.json`))  
let fil = vote.map(v => v.participant)
let id_vote = sender ? sender : '94755183063@s.whatsapp.net'
if(fil.includes(id_vote)) {
...
await limitAdd(sender, limit)
		...
		if (!isQuotedSticker) return reply('```Reply stc nya```')
		            ...
		            dua = typeof anu[1] !== 'undefined' ? anu[1] : `кαвєє σƒ¢`
...
break
		...
		if (!isQuotedSticker) return reply('Reply stc nya!')
					...
					fakethumb(buffer,'```Nih kak, jgn lupa Support YT : кαвєє σƒ¢```')
					fs.unlinkSync(ran)
					})
					await limitAdd(sender, limit)
					break   
        ...
        break;
           ...
           case 'getpp':
				if (mek.message.extendedTextMessage != undefined){
					...
					anuu  = {contextInfo:{"forwardingScore":999,"isForwarded":true,'stanzaId': "B826873620DD5947E683E3ABE663F263", 'participant': `${numbernye}@s.whatsapp.net`, 'remoteJid': '6289523258649-1604595598@g.us', 'quotedMessage': {"imageMessage": {"caption": `「 Bot by КАѴЄЄ 」`, 'jpegThumbnail': fs.readFileSync(`image/${thumbnail}`)}}}}
...
if (budy.includes("eror",'error','Eror','Error')){
					...
					alpha.sendMessage(from, daieeeee, sticker, { contextInfo: {mentionedJid: [sender]}, quoted: { "key": { "participant": `${numbernye}@s.whatsapp.net`, "remoteJid":  '6283136505591-1614953337@g.us', "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": fs.readFileSync(`image/${thumbnail}`), "mimetype": "application/octet-stream","title": "YT : кαвєє σƒ¢", "fileLength": "36", "pageCount": 0, "fileName": `Kenapa bisa error???`}}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		        }
